...
(function () {
  ...
  // ── ADMIN AUTH ───────────────────────────────────────────────────
  // Admin login verifies via Supabase Auth then checks the admin_users
  // table. There is NO hardcoded password fallback.
  window.adminDoLogin = async function () {
    const client = sb();
    if (!client) {
      _showAdminErr('Supabase not connected. Admin login unavailable.');
      return;
    }
    ...
    _clearAdminErr();
    if (!email || !pass) {
      _showAdminErr('Enter email and password.');
      return;
    }
    ...
    try {
      ...
      // Verify the account exists in admin_users table.
      ...
      // Build the admin session object (kept in memory only).
      const name = adminRow.name || authData.user.user_metadata?.full_name || email.split('@')[0];
      window.adminSession = {
      ...
      if (typeof window.showToast   === 'function') window.showToast('Welcome to Admin Panel 🛡️', 'success');
      if (typeof window.logAdminActivity === 'function')
        window.logAdminActivity(`Admin login: ${email}`, 'green');

    } catch (err) {
      _showAdminErr(err.message || 'Authentication failed.');
    } finally {
      if (btn) {
        btn.disabled = false;
  ...
  window.adminLogout = function () {
    ...
    if (typeof window.logAdminActivity === 'function' && window.adminSession)
      window.logAdminActivity(`Admin logged out: ${window.adminSession.email}`, 'red');
    window.adminSession = null;
    const client = sb();
    if (client) client.auth.signOut();
    if (typeof window.navigate  === 'function') window.navigate('home');
    if (typeof window.showToast === 'function') window.showToast('Admin session ended.', 'info');
  };
  ...
  function _clearAdminErr() {
    const box = document.getElementById('adminLoginError');
    if (box) box.style.display = 'none';
  }
  function _updateAdminTopbar() {
    const s = window.adminSession;
    if (!s) return;
    const nameEl   = document.getElementById('adminTopbarName');
    const emailEl  = document.getElementById('adminTopbarEmail');
    const avatarEl = document.getElementById('adminTopbarAvatar');
