...
//   USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
//
// -- purchased_pdfs already exists — ensure email column is indexed:
...
(function () {
  ...
  window.toggleWishlistItem = async function toggleWishlistItem(id) {
    ...
    // Update the specific heart button immediately
    ...
    try {
      if (inWish) {
        ...
        if (delError) {
          ...
          if (typeof showToast === 'function') showToast('Failed to remove — please try again.', 'error');
        }
      } else {
        // UPSERT
        ...
        if (upsertError) {
          console.error('❌ Wishlist upsert failed:', {
          ...
          if (verifyErr || !verifyData || verifyData.length === 0) {
            console.error(
              '❌ WISHLIST VERIFICATION FAILED — row not found after upsert!',
              '\n  user_id:', userId,
              '\n  pdf_id:', pdfId,
              '\n  verifyErr:', verifyErr?.message ?? 'none',
              '\n  rows returned:', verifyData?.length ?? 0,
              '\n  CAUSE: likely RLS policy blocking SELECT, or wrong user_id/pdf_id type mismatch'
            );
          } else {
