...
(function () {
  ...
  function _buildPremiumCard(pdf) {
    ...
    var coverHtml = cover
      ...
      : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:2rem;background:linear-gradient(135deg,rgba(125,17,34,0.08),rgba(139,92,246,0.08))">&#x1F4CC;</div>';
  ...
  function _buildPremiumSection(pdfs, status) {
    ...
    return '<div id="' + SECTION_ID + '" style="margin-top:24px;padding-top:24px;border-top:1px solid var(--glass-border,rgba(255,255,255,0.08))">'
      ...
      + 'background:none;border:1px solid rgba(125,17,34,0.25);border-radius:20px;'
  ...
  window.renderAdminPremiumCategories = async function(container) {
    ...
    try {
      ...
      h += '<div style="margin-top:14px;padding:10px 14px;border-radius:8px;background:rgba(125,17,34,.08);'
        + 'border:1px solid rgba(125,17,34,.2);font-size:.72rem;color:rgba(255,255,255,.5);line-height:1.5">'
  ...
  async function renderPremiumLibraryPage(force) {
    ...
    var _safetyTo = setTimeout(function() {
      container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text2)">'
        ...
        + '<button onclick="window.SMCI.renderPremiumLibraryPage(true)" style="background:linear-gradient(135deg,#7d1122,#0ea5e9);color:#fff;font-weight:700;padding:10px 24px;border-radius:20px;border:none;cursor:pointer;font-size:.85rem">↻ Retry</button>'
        + '</div>';
    }, 5000);
