...
/* (old image-based banner CSS removed — now using CSS+SVG grid design) */
.arena-promo-bg-pattern{
  position:absolute;
  inset:0;
  overflow:hidden;
  pointer-events:none;
...
/* Row 1: left player | center title | right player */
.arena-promo-side{
  display:flex;
...
.arena-promo-left{grid-column:1}
.arena-promo-right{grid-column:3}
.arena-promo-avatar{
  ...
  box-shadow:0 2px 12px rgba(201,154,60,0.12);
  transition:transform .25s ease,box-shadow .25s ease;
}
.arena-promo-banner:hover .arena-promo-avatar{
  transform:scale(1.08);
  box-shadow:0 4px 18px rgba(201,154,60,0.25);
}
.arena-promo-avatar svg{width:28px;height:28px}
.arena-promo-avatar-label{
  font-size:9px;
  font-weight:600;
  color:rgba(245,233,224,0.45);
...
.arena-promo-vs{
  ...
  font-weight:800;
  color:#c99a3c;
  text-shadow:0 0 14px rgba(201,154,60,0.5);
  font-style:italic;
...
.arena-promo-vs::before,.arena-promo-vs::after{
  content:'';
  position:absolute;
  top:50%;
  width:16px;
...
.arena-promo-vs::before{left:-18px}
.arena-promo-vs::after{right:-18px}
.arena-promo-center{
  grid-column:1/-1;
  grid-row:2;
  display:flex;
...
.arena-promo-title .arena-promo-sword{
  font-size:1.25rem;
  filter:drop-shadow(0 1px 4px rgba(201,154,60,0.4));
}
.arena-promo-sub{
  font-size:0.72rem;
  font-weight:600;
  color:rgba(201,154,60,0.88);
  text-transform:uppercase;
  letter-spacing:2px;
}
.arena-promo-modes{
...
.arena-promo-banner:hover .arena-promo-mode-pill{
...
.arena-promo-badge-icon{font-size:0.78rem;filter:grayscale(0.2)}
.arena-promo-footer{
  grid-column:1/-1;
  grid-row:4;
  display:flex;
...
.arena-promo-cta{
  ...
  box-shadow:0 3px 12px rgba(201,154,60,0.35);
  letter-spacing:0.02em;
}
.arena-promo-banner:hover .arena-promo-cta{
  transform:scale(1.06);
  box-shadow:0 5px 18px rgba(201,154,60,0.5);
}
.arena-promo-banner:active .arena-promo-cta{
...
@media(max-width:600px){
  .arena-promo-grid{padding:18px 16px 14px;gap:3px}
  .arena-promo-avatar{width:40px;height:40px}
  .arena-promo-avatar svg{width:24px;height:24px}
  .arena-promo-avatar-label{font-size:8px}
  .arena-promo-vs{font-size:16px}
  .arena-promo-vs::before,.arena-promo-vs::after{width:10px}
  .arena-promo-vs::before{left:-12px}
  .arena-promo-vs::after{right:-12px}
  .arena-promo-title{font-size:1.02rem}
  .arena-promo-title .arena-promo-sword{font-size:1.05rem}
  .arena-promo-sub{font-size:0.6rem;letter-spacing:1.2px}
  .arena-promo-modes{gap:4px}
  .arena-promo-mode-pill{font-size:0.58rem;padding:2px 7px}
  .arena-promo-badges{gap:10px}
  .arena-promo-badge{font-size:0.56rem}
  .arena-promo-badge-icon{font-size:0.7rem}
  .arena-promo-tagline{font-size:0.66rem}
  .arena-promo-cta{font-size:0.72rem;padding:7px 14px}
  .arena-promo-footer{padding:6px 4px 2px}
}
@media(max-width:480px){
  .arena-promo-grid{padding:16px 12px 12px}
  .arena-promo-avatar{width:34px;height:34px;border-width:1.5px}
  .arena-promo-avatar svg{width:20px;height:20px}
  .arena-promo-avatar-label{display:none}
  .arena-promo-vs{font-size:14px}
  .arena-promo-vs::before,.arena-promo-vs::after{display:none}
  .arena-promo-title{font-size:0.92rem}
  .arena-promo-title .arena-promo-sword{font-size:0.95rem}
  .arena-promo-sub{font-size:0.55rem;letter-spacing:1px}
  .arena-promo-mode-pill{font-size:0.54rem;padding:2px 6px;border-radius:5px}
  .arena-promo-badges{gap:8px}
  .arena-promo-badge{font-size:0.52rem}
  .arena-promo-badge-icon{font-size:0.65rem}
  .arena-promo-tagline{font-size:0.62rem}
  .arena-promo-cta{font-size:0.68rem;padding:6px 12px;border-radius:10px}
}
@media(max-width:380px){
  .arena-promo-grid{padding:14px 10px 10px;gap:2px}
  .arena-promo-avatar{width:28px;height:28px}
  .arena-promo-avatar svg{width:16px;height:16px}
  .arena-promo-vs{font-size:12px}
  .arena-promo-title{font-size:0.84rem;gap:5px}
  .arena-promo-title .arena-promo-sword{font-size:0.88rem}
  .arena-promo-sub{font-size:0.5rem;letter-spacing:0.5px}
  .arena-promo-modes{gap:3px}
  .arena-promo-mode-pill{font-size:0.5rem;padding:2px 5px}
  .arena-promo-badges{gap:6px}
  .arena-promo-badge{font-size:0.48rem}
  .arena-promo-badge-icon{font-size:0.6rem}
  .arena-promo-tagline{font-size:0.56rem}
  .arena-promo-cta{font-size:0.62rem;padding:5px 10px}
  .arena-promo-footer{gap:6px}
}
/* Very small phones — stack footer vertically */
@media(max-width:320px){
  .arena-promo-footer{flex-direction:column;gap:6px;align-items:center;text-align:center}
  .arena-promo-tagline{font-size:0.54rem}
}
`;
...
function init(){
  ...
  BrainLab.renderPracticeArena=function(){
    ...
    // Premium Arena promo banner — CSS+SVG design, entire banner is clickable
    var bannerHTML='<a href="javascript:void(0)" class="arena-promo-banner" onclick="window.Arena.showHome()" role="button" aria-label="Enter Studyria Arena — real-time competitive learning: 1v1, Team Battles and Free-for-All quiz battles">'+
      '<div class="arena-promo-inner">'+
        // ── Decorative background pattern (subtle quiz/battle elements) ──
        '<div class="arena-promo-bg-pattern" aria-hidden="true">'+
          '<svg viewBox="0 0 400 200" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg" style="position:absolute;inset:0;width:100%;height:100%;opacity:0.04">'+
            '<circle cx="50" cy="40" r="30" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
            '<circle cx="350" cy="160" r="40" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
            '<rect x="280" y="20" width="60" height="40" rx="4" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
            '<rect x="60" y="140" width="50" height="35" rx="4" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
            '<path d="M180 20 L220 20 L220 60 L260 60 L260 100 L220 100 L220 140 L180 140 L180 100 L140 100 L140 60 L180 60 Z" fill="#c99a3c"/>'+
            '<circle cx="320" cy="100" r="15" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
            '<path d="M20 180 L60 120 L100 180" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
            '<path d="M340 40 L380 100 L300 100" fill="none" stroke="#c99a3c" stroke-width="0.5"/>'+
          '</svg>'+
        '</div>'+
        // ── Main grid layout: left player | center content | right player ──
        '<div class="arena-promo-grid">'+
          // Left player
          '<div class="arena-promo-side arena-promo-left">'+
            '<div class="arena-promo-avatar">'+
              '<svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">'+
                '<circle cx="20" cy="15" r="7" stroke="#c99a3c" stroke-width="2"/>'+
                '<path d="M8 36c0-7 5-12 12-12s12 5 12 12" stroke="#c99a3c" stroke-width="2" stroke-linecap="round"/>'+
              '</svg>'+
            '</div>'+
            '<span class="arena-promo-avatar-label">Player 1</span>'+
          '</div>'+
          // VS element
          '<div class="arena-promo-vs">VS</div>'+
          // Right player
          '<div class="arena-promo-side arena-promo-right">'+
            '<div class="arena-promo-avatar">'+
              '<svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">'+
                '<circle cx="20" cy="15" r="7" stroke="#c99a3c" stroke-width="2"/>'+
                '<path d="M8 36c0-7 5-12 12-12s12 5 12 12" stroke="#c99a3c" stroke-width="2" stroke-linecap="round"/>'+
              '</svg>'+
            '</div>'+
            '<span class="arena-promo-avatar-label">Player 2</span>'+
          '</div>'+
          // Center content (overlaps both sides)
          ...
          '</div>'+
          // Badges (trophy, streak, rating)
          '<div class="arena-promo-badges">'+
            '<span class="arena-promo-badge"><span class="arena-promo-badge-icon">🏆</span> Rating</span>'+
            '<span class="arena-promo-badge"><span class="arena-promo-badge-icon">🔥</span> Streak</span>'+
            '<span class="arena-promo-badge"><span class="arena-promo-badge-icon">⚡</span> Live</span>'+
          '</div>'+
          // Footer: tagline + CTA
          '<div class="arena-promo-footer">'+
            '<span class="arena-promo-tagline">Practice smarter. Compete harder. Improve faster.</span>'+
