...
async function showHome(){
  ...
  // Check for active match recovery (refresh/reconnect)
  var recovery=await api('recoverMatch',{userId:S.user.id});
  if(recovery.ok&&recovery.matchId&&recovery.match){
    var matchStatus=recovery.matchStatus;
    var myPlayer=recovery.match.players.find(function(p){return p.userId===S.user.id;});
    if(myPlayer&&myPlayer.status==='completed'&&matchStatus==='completed'){
      // Match completed — show results
      S.matchId=recovery.matchId;
      S.match=recovery.match;
      S.cfg={mode:recovery.match.mode,qCount:recovery.match.questionCount,exam:recovery.match.exam,cat:recovery.match.category,diff:recovery.match.difficulty};
      showResults(JSON.parse(recovery.match.winner||'{}'));
      return;
    }else if(myPlayer&&myPlayer.status==='abandoned'){
      // Player abandoned — clear and show home
      api('leaveMatch',{matchId:recovery.matchId,userId:S.user.id});
    }else if(matchStatus==='lobby'){
      // Match in lobby — rejoin
      S.matchId=recovery.matchId;
      S.match=recovery.match;
      S.cfg={mode:recovery.match.mode,qCount:recovery.match.questionCount,exam:recovery.match.exam,cat:recovery.match.category,diff:recovery.match.difficulty};
      toast('Rejoining your arena lobby...');
      showLobby();
      startPresence();
      return;
    }else if(matchStatus==='in_progress'){
      // Match in progress — rejoin battle
      S.matchId=recovery.matchId;
      S.match=recovery.match;
      S.cfg={mode:recovery.match.mode,qCount:recovery.match.questionCount,exam:recovery.match.exam,cat:recovery.match.category,diff:recovery.match.difficulty};
      var myStatus=myPlayer?myPlayer.status:'lobby';
      if(myStatus==='completed'){
        // Already completed — show waiting
        showWaitingForOpponent();
      }else{
        // Still playing — restore battle
        S.battle.questions=getMatchQuestions(S.match);
        S.battle.qIdx=myPlayer&&myPlayer.answers?myPlayer.answers.filter(function(a){return a;}).length:0;
        S.battle.answers=[];
        S.battle.correct=0;S.battle.wrong=0;S.battle.skipped=0;
        S.battle.topicStats={};S.battle.totalTime=0;
        S.battle.startTime=Date.now();
        S.battle.qStart=Date.now();
        toast('Restoring your battle...');
        startBattle();
      }
      startPresence();
      return;
...
async function startSearch(){
  ...
  // Update presence
  await api('ping',{userId:S.user.id,userName:S.user.name,exam:S.cfg.exam,status:'in_arena'});
...
async function searchPoll(){
  ...
  var res=await api('search',{userId:S.user.id,exam:S.cfg.exam||'All'});
...
function answerQ(optIdx){
  ...
  // Submit to backend
  api('submitAnswer',{matchId:S.matchId,userId:S.user.id,questionIndex:idx,answer:String.fromCharCode(97+optIdx),isCorrect:isCorrect,timeSpent:timeSpent});
...
async function waitingPoll(){
  ...
  // Use getMatchResult to get the finalized snapshot if available
  var res=await api('getMatchResult',{matchId:S.matchId});
  if(!res.ok)return;
  S.match=res.match;
  
  if(res.finalized){
    stopTimer('waitingPoll');
    showResults(res.result?res.result.winner:JSON.parse(S.match.winner||'{}'));
    return;
  }
...
async function showResults(winner){
  ...
  // Get final match state — use getMatchResult for the authoritative finalized snapshot
  var res=await api('getMatchResult',{matchId:S.matchId});
  if(res.ok){
    S.match=res.match;
    if(res.finalized&&res.result){
      // Use the immutable finalized result as the source of truth
      var finalizedPlayers=res.result.playerResults;
      if(finalizedPlayers&&finalizedPlayers.length){
        // Merge finalized player results into match players for display
        S.match.players=S.match.players.map(function(p){
          var fp=finalizedPlayers.find(function(f){return f.userId===p.userId;});
          if(fp){
            p.score=fp.score; p.correct=fp.correct; p.wrong=fp.wrong; p.skipped=fp.skipped;
            p.totalTime=fp.totalTime; p.timing=fp.timing||p.timing;
            p.topicBreakdown=fp.topicBreakdown||p.topicBreakdown;
          }
          return p;
        });
      }
  ...
  // Performance analysis — SERVER-AUTHORITATIVE DATA
  // Use me.correct/wrong/skipped from server-recalculated match data
  var sCorrect=me?me.correct||0:0;
  var sWrong=me?me.wrong||0:0;
  var sSkipped=me?me.skipped||0:0;
  var sScore=me?me.score||0:0;
  var sTotalTime=me?me.totalTime||totalTime:totalTime;
  ...
  h+='<div class="arena-result-row"><span>Correct</span><span>'+sCorrect+'</span></div>';
  h+='<div class="arena-result-row"><span>Wrong</span><span>'+sWrong+'</span></div>';
  h+='<div class="arena-result-row"><span>Skipped</span><span>'+sSkipped+'</span></div>';
  h+='<div class="arena-result-row"><span>Accuracy</span><span>'+Math.round((sCorrect/(sCorrect+sWrong||1))*100)+'%</span></div>';
  h+='<div class="arena-result-row"><span>Total Time</span><span>'+fmtTime(sTotalTime)+'</span></div>';
  h+='<div class="arena-result-row"><span>Avg Time/Q</span><span>'+fmtTime(sTotalTime/(total||1))+'s</span></div>';
  
  // Timing analysis — use server-authoritative timing array if available
  var times=(me&&me.timing&&me.timing.length)?me.timing:S.battle.answers.filter(function(a){return a;}).map(function(a){return a.timeSpent;});
  if(times.length){
  ...
  if(window.BrainLab){
    BrainLab.saveSession({
      ...
      total_questions:total,correct_count:sCorrect,wrong_count:sWrong,skipped_count:sSkipped,
      score:sScore,time_taken:sTotalTime,
